...
Streamlit 製の可視化・データ整備ツールです。  
故障データの分析から予測までを一貫して行えます。

### 主な機能
- Excel → JSON 変換
- ビル名／装置種別ごとの集計
- Plotly による年間・月次の故障件数可視化
- しきい値ベースの異常検知
- 装置種別ごとの交換部品予測

📁 実装ファイル  
`upload_files/failure_dashboard/dashboard.py`
...
故障内容から交換部品を推定する  
**VotingClassifier ベースの学習・保存スクリプト群** です。

### 主な機能
- データ前処理
- 特徴量エンジニアリング
- アンサンブル学習（VotingClassifier）
- 学習済みモデルの保存（joblib）

📁 実装ファイル  
`upload_files/create_model/save_extended_model.py`
